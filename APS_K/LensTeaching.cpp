...
BOOL CMotorDlg::OnInitDialog()
{
	...
	GetDlgItem(IDC_BUTTON_GOTO_LENSLASERALIGNMOTOR)->ShowWindow(SW_HIDE);
...
#else
			if (bThreadTaskReadyRun == false && i != Motor_Lens_Xt&& i != Motor_Lens_Yt && i != Motor_Lens_TH)
#endif
...
#if (__MACHINE_MODEL == MACHINE_1ST)
		for (i = Motor_Lens_X; i <= Motor_Lens_Z; i++)
#else
		for (i = Motor_Lens_X; i <= Motor_Lens_TH; i++)
#endif
