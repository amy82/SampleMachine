    ...
    if (!fopen_s(&out, fileName, "a"))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
					fprintf_s(out, "%lf,", Task.SFR.dMaxSfr_StepLog[i]);	//saveSfrLog
...
bool getMotorPos(int axis, int index, double &pos, bool flagDisp)		// flagDisp :1-> 화면 보기 용  0-> 기계치값
{
...
#else
    if (axis >= Motor_Lens_X && axis <= Motor_Lens_TH)			// 5축 모터
    {
        if (index >= Wait_Pos && index <= Bonding_Pos)			// 0:대기, 1:공급, 2:Align, 3:본딩
        {
            if (flagDisp)
    ...
    else if (axis >= Motor_PCB_X && axis <= Motor_PCB_Yt)		//5축 모터
    {
        if (flagDisp)
            pos = (model.axis[axis].pos[index] - sysData.dDataset[axis]);
        else
	...
	else if (axis >= Motor_LensAlign_X && axis <= Motor_LensLaser_Xt)		// LensLaserAlign
	{
		if (flagDisp)
			pos = (model.axis[axis].pos[index] - sysData.dDataset[axis]);
		else
	...
	else if (axis == Motor_Insp_X)		//INSP X 축
	{
		if (flagDisp)
			pos = (model.axis[axis].pos[index] - sysData.dDataset[axis]);
		else
