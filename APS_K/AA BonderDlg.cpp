...
UINT Thread_TaskOrigin(LPVOID parm)
{
	...
	bHomeComplete = motor.HomeProcessAll_1st();
#else
	bHomeComplete = motor.HomeProcessAll_2st();
#endif
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////ㅋㅋㅋ
		...
		else if ( Task.PCBTask >= 11000 && Task.PCBTask < 49000 )
		{
			...
			}else
			{
				...
				else if (Task.PCBTask >= 39000 && Task.PCBTask < 40000)		Task.PCBTask = theApp.MainDlg->pcbProcess.RunProc_InspAAPos(Task.PCBTask);							//다크,디펙검사
#if (__MACHINE_MODEL == MACHINE_1ST)
				else if( Task.PCBTask >= 40000 && Task.PCBTask < 41000 )	Task.PCBTask = theApp.MainDlg->pcbProcess.RunProc_LensLoading(Task.PCBTask);						//렌즈 로딩
#endif
#endif
...
BOOL CAABonderDlg::OnInitDialog()
{
	...
	//LGIT_MODEL_INDEX = M2_RFPCB_5M;// M1_HEATING_GEN2;		//M2_RFPCB_5M		==========================================================================
	...
	{
		LightControlSecond.ctrlLedVolume(0, model.m_iLedValue[LEDDATA_TOP1_CHART]);
	}
	else
	{
...
void CAABonderDlg::Rs232Init()
{
	...
	//
	//----------------------------------------------------------------------------------------------------------------
	//sysData.iCommPort[COMM_LIGHT1], sysData.iBaudRate[COMM_LIGHT1]
	bRet_Con_RS232C = OCControl.OpenLightSerial(sysData.iCommPort[COMM_LIGHTOC_1ST], 115200);
	if (bRet_Con_RS232C == true)
	{
		OCControl.SendLightONOFF(true);
		Sleep(700);
		OCControl.SendLightLxStep(model.m_iLedValue[LEDDATA_6500K]);
		//_stprintf_s(szLog, SIZE_OF_1K, _T("[Serial]통합광원 연결 완료 COM%d"), g_clSysData.m_nOCPort[0]);
		logStr.Format("	[Serial]통합광원 연결 완료 COM%d", sysData.iCommPort[COMM_LIGHTOC_1ST]);
	}
	else
	{
		//_stprintf_s(szLog, SIZE_OF_1K, _T("[Serial]통합광원 연결 실패 COM%d"), g_clSysData.m_nOCPort[0]);
		logStr.Format("	[Serial]통합광원 연결 실패 COM%d", sysData.iCommPort[COMM_LIGHTOC_1ST]);
	}
	putListLog(logStr);
	...
	{
		//조명 IR CHART
		sCommPort.Format("COM%d", sysData.iCommPort[COMM_LIGHT4]);
		LightControlSecond.SetReceiveProcPtr(this);
		bRet_Con_RS232C = LightControlSecond.Connect_Device(sCommPort, 0);
	}
	...
	//조명 4 RIGHT SIDE
	sCommPort.Format("COM%d", sysData.iCommPort[COMM_LIGHT2]);
	LightControlFourth.SetReceiveProcPtr(this);
	bRet_Con_RS232C = LightControlFourth.Connect_Device(sCommPort, 0);

	//LEFT SIDE CHART
	sCommPort.Format("COM%d", sysData.iCommPort[COMM_LIGHT5]);
	LightControlFifth.SetReceiveProcPtr(this);
	bRet_Con_RS232C = LightControlFifth.Connect_Device(sCommPort, 0);
	...
	UVControl.Altis_UV_Connect(0);
	Sleep(500);
	UVControl.Altis_UV_Connect(1);
	Sleep(500);
	UVControl.Altis_UV_Connect(2);
	Sleep(500);
	UVControl.Altis_UV_Connect(3);
	Sleep(500);
	UVControl.Altis_UV_Connect(4);
	Sleep(500);
#endif
...
void CAABonderDlg::SetInterface_Label()
{
	...
	{
		temp.Format(_T("M1_HEATING_GEN2"));
		ModelSelect = true;
	}
	else if (LGIT_MODEL_INDEX == M1_LSI_5M)
	{
		temp.Format(_T("M1_LSI_5M"));
		ModelSelect = true;
	}
#else
	if (LGIT_MODEL_INDEX == M2_RFPCB_5M)
	{
		temp.Format(_T("M2_RFPCB_5M"));
		ModelSelect = true;
	}
	else if (LGIT_MODEL_INDEX == M2_FF_MODULE)
	{
		temp.Format(_T("M2_FF_MODULE"));
		ModelSelect = true;
	}
	else if (LGIT_MODEL_INDEX == M2_OV9284_5M)
	{
		temp.Format(_T("M2_OV9284_5M"));
		ModelSelect = true;
	}
	else if (LGIT_MODEL_INDEX == M2_VB1940_5M)
	{
...
void CAABonderDlg::SetInterface_CreateDlg()
{
	...
	if (motorDlg4 == NULL) {
		motorDlg4 = new CMotorDlg4;
		motorDlg4->Create(IDD_DIALOG_LENS_LASER_ALIGN_TEACHING);
...
void CAABonderDlg::ctrlSubDlg(int iDlgNo)
{
	...
	if ((iDlgNo == MOTOR_DLG && motorDlg2->IsWindowVisible()) ||
		...
		(iDlgNo == MOTOR_DLG4 && motorDlg4->IsWindowVisible()))
	{
...
bool CAABonderDlg::calcAlignData(int camNo, int iMarkType, double &dOffsetX, double &dOffsetY, double &dOffsetTh)
{
	...
	if(camNo == 1)
	{
		dOffsetX = rotatePos.x * alignDic;
		dOffsetY	=	rotatePos.y * alignDic;
...
int CAABonderDlg::_getMTF(int Mode, bool LogView)
{
	...
	//
	double fMax_SFR_N4[COMMON_MTF_INSP_CNT] = {0.0, };  //MTF_INSP_CNT
	//SFR_COMP_MAX
	bool bSfrMaxTest = true;
	if ( model.strInfo_CCM.m_iCnt_Average_Thru_Focus < 1 )
	{
	...
	//if(Mode==SFR_FINAL)
	//{
	//	iCnt_Focus=model.strInfo_CCM.m_iCnt_Average_Fine_Focus;		//step 반복 횟수
	//}
	//else
	//{
	//	iCnt_Focus=model.strInfo_CCM.m_iCnt_Average_Thru_Focus;		//mtf uv전 , 후 , 파이널 한번만 찍을 때 사용
	//}
	//==============================================================================================================================================
	//==============================================================================================================================================
	//new
...
//#ifndef SFR_COMP_MAX
	if (bSfrMaxTest == false)
	{
		for (int i = 0; i < model.mGlobalSmallChartCount; i++)
		{
			...
			//strView.Format("SFR Avr [%d]  =  %lf", i, fMax_SFR_N4[i]);
			_stprintf_s(szLog, SIZE_OF_1K, _T("SFR Avr [%d]  =  %lf"), i, fMax_SFR_N4[i]);
			putListLog(szLog);
...
void CAABonderDlg::ChipIDCountPlus()
{
	...
	if (count2 - count1 > 1 && count1 > 0)
	{
		...
		// "SHM150_" 다음에 나오는 숫자 추출

		const char* prefix = "SHM100_";
		size_t prefixLength = strlen(prefix);

		if (strncmp(Task.ChipID, prefix, prefixLength) == 0) {
			const char* numberStr = Task.ChipID + prefixLength;
			number = atoi(numberStr);
		}

		number++;

		sprintf(Task.ChipID, "%s%03d", prefix, number);
	}
	count1 = model.mCurModelName.Find(_T("150"));
	count2 = model.mCurModelName.ReverseFind('150');
	if (count2 - count1 > 1 && count1 > 0)
	{
...
void CAABonderDlg::OnStnClickedLabelId()
{
	...
	if (count2 - count1 > 1 && count1 > 0)
	{
		sprintf_s(Task.ChipID, "SHM100_");
	}
	count1 = model.mCurModelName.Find(_T("150"));
	count2 = model.mCurModelName.ReverseFind('150');
	if (count2 - count1 > 1 && count1 > 0)
	{
		sprintf_s(Task.ChipID, "SHM150_");
	}
